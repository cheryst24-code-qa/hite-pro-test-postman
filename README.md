...
Тесты написаны в **Postman**, запускаются через **Newman** и могут быть интегрированы в CI/CD (например, GitHub Actions).
...
```
▶️ Запуск локально
Создайте файл environment.json на основе шаблона:
...
💡 Тесты автоматически определяют ID устройств по типам (dimmer, motion, temperature и др.) и пропускают шаги, если устройство отсутствует.
